...
3. **Configure the registration:**
   - **Name**: Enter a descriptive name
     - Example: "Hermes Agent Microsoft Graph Integration"
...
**Store these safely** - you'll add them to Hermes Agent configuration later.
...
3. **Configure the secret:**
   - **Description**: Enter a description
     - Example: "Hermes Agent production secret"
...
Now that Azure setup is complete, configure Hermes Agent with your credentials.
...
### Option B: Hermes Agent Config File

Edit your Hermes Agent configuration (`~/.hermes-agent/hermes-agent.json` or via `/config` command):
...
- Restrict file permissions: `chmod 600 ~/.hermes-agent/hermes-agent.json`

## Step 9: First Authentication

When you first use the Microsoft Graph skill:

1. Hermes Agent will display a device code and URL
...
6. Return to Hermes Agent - authentication complete!

**Token storage:** Tokens are saved to `~/.hermes-agent/auth/microsoft-graph.json` and automatically refreshed.
...
- ✅ Credentials added to Hermes Agent config
...
2. Update Hermes Agent configuration with new secret
...
**Solution:**
1. Delete `~/.hermes-agent/auth/microsoft-graph.json`
...
1. **Rotate secrets regularly**
   ...
   - Update Hermes Agent config with new secret
...
4. **Secure token storage**
   - Tokens stored at `~/.hermes-agent/auth/microsoft-graph.json`
...
3. Consult Hermes Agent documentation
